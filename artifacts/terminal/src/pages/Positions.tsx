...
import { TrendingUp, TrendingDown, Clock, ExternalLink, X, Trash2, Pencil, ChevronDown, ChevronUp, ShieldCheck, ShieldAlert, Download, Target } from "lucide-react";
  ...
  return (
    <div className="px-3 py-4 space-y-4 pb-6">
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <div className="bg-[#0d0d18] border border-white/6 rounded-xl p-3 text-center">
          <p className="text-xl font-black text-white">{closedPositions.length}</p>
          ...
          {closedPositions.length > 0 && (
            <div className="flex items-center justify-between">
              <div className="text-[10px] text-white/30">
                {closedPositions.length} trades · {closedPositions.filter(p => (p.pnlSol ?? 0) >= 0).length}W / {closedPositions.filter(p => (p.pnlSol ?? 0) < 0).length}L
              </div>
