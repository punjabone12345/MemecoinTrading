...
import {
  ...
  useDeleteClosedTrade, useAutoTraderStatus,
} from "@/lib/api";
...
import {
  ...
  Trophy, ThumbsUp, ThumbsDown, Zap, Activity, Flame,
} from "lucide-react";
...
function fmtSigned(v: number, d = 4) {
  ...
  return (
    <div className={`rounded-xl border p-3.5 ${priorityColor}`}>
      ...
      <div className="flex items-center gap-2 mb-2 text-xs">
        ...
        <span className="ml-auto text-white/30 text-[10px]">{s.confidence}% conf</span>
      </div>
...
function StatCard({ icon, bg, label, value, valueClass, sub }: {
  icon: React.ReactNode; bg: string; label: string; value: string; valueClass: string; sub?: string;
}) {
  return (
      ...
      <div className="grid grid-cols-3 gap-1.5">
        {[
          ...
          <div key={label} className={`rounded-lg p-2 text-center ${pass ? "bg-emerald-500/10" : "bg-white/3"}`}>
            <div className={`font-black text-sm ${pass ? "" : "text-white/40"}`}>{val}</div>
            <div className={`text-[9px] ${pass ? "opacity-70" : "text-white/30"}`}>{label} {pass ? "✓" : "✗"}</div>
          </div>
        ))}
      </div>
      <p className="text-[9px] opacity-40 mt-2 text-right">{passCount}/3 conditions met · refreshes every 30m</p>
    </div>
  );
}

function LlmBadge({ verdict, provider }: { verdict?: string; provider?: string }) {
  if (!verdict || verdict === "none") return null;
  const color = verdict === "TRADE" ? "bg-emerald-500/15 text-emerald-400 border-emerald-500/25"
    : verdict === "RISKY"  ? "bg-amber-500/15 text-amber-400 border-amber-500/25"
    : "bg-red-500/15 text-red-400 border-red-500/25";
  const short = provider === "groq" ? "G" : provider === "heuristic" ? "H" : provider?.charAt(0)?.toUpperCase() ?? "";
  return (
    <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-black border ${color}`}>
...
function NoteModal({
  symbol, closedAt, note: initialNote, positionId,
  onSave, onClose, isPending,
}: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-[#0d0d18] border border-amber-500/30 rounded-2xl p-5 w-full max-w-sm space-y-4" onClick={e => e.stopPropagation()}>
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-white font-bold text-base">Edit Note</h2>
        </div>
        <p className="text-white/50 text-xs">${symbol} — {toIST(closedAt)}</p>
        <div>
        ...
        <div className="flex gap-2">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl bg-white/8 text-white/50 text-sm font-semibold active:scale-95">Cancel</button>
          <button
            onClick={() => onSave(positionId, note)}
            disabled={isPending}
...
function JournalEntryCard({
  entry, onDelete, onEdit, isDeleting,
}: {
  ...
  return (
    <div className={`bg-[#0d0d18] border rounded-xl p-3.5 ${isWin ? "border-emerald-500/15" : "border-red-500/10"}`}>
      ...
      <div className="grid grid-cols-3 gap-2 text-[10px] mb-2 text-white/40">
        <div>Score <span className="text-white/70 font-bold">{entry.aiScore}</span></div>
        <div>Conf <span className="text-white/70 font-bold">{entry.confidence}%</span></div>
        <div>Liq <span className="text-white/70 font-bold">{formatMcap(entry.entryLiquidityUsd)}</span></div>
      </div>
        ...
        <div className="mt-2 space-y-1">
          {entry.warnings.map((w, i) => (
            <div key={i} className="flex items-start gap-1.5 text-[10px] text-white/50">
              {isWin ? <ThumbsUp className="w-3 h-3 text-emerald-400/60 shrink-0 mt-0.5" />
      ...
      <div className="flex items-center justify-end gap-2 mt-2 pt-2 border-t border-white/5">
        ...
        {confirmDelete ? (
          <div className="flex items-center gap-1.5">
            ...
            <button onClick={() => setConfirmDelete(false)} className="text-[10px] font-bold px-2 py-1 rounded-md bg-white/8 text-white/50 active:scale-95">
              No
            </button>
          </div>
        ) : (
  ...
  if (data.totalTrades === 0) {
    return (
      <div className="px-4 py-8 text-center">
        ...
        <p className="text-white/20 text-xs mt-1">Every trade will be automatically analyzed here.</p>
      </div>
    );
  }

  const displayedEntries = filter === "wins" ? data.recentWins
    : filter === "losses" ? data.recentLosses
    : data.allEntries;

  // Use per-type tag percentages so "% of wins" and "% of losses" make sense
  const winTagRows = data.totalWins > 0
    ? Object.entries(data.winTagPercentage ?? data.tagPercentage)
        .filter(([tag]) => WIN_TAGS.includes(tag))
        .sort((a, b) => b[1] - a[1])
        .map(([tag, pct]) => ({ tag, label: TAG_LABELS[tag]?.label ?? tag, pct }))
    : [];

  const lossTagRows = data.totalLosses > 0
    ? Object.entries(data.lossTagPercentage ?? data.tagPercentage)
        .filter(([tag]) => !WIN_TAGS.includes(tag))
        .sort((a, b) => b[1] - a[1])
        .slice(0, 8)
        .map(([tag, pct]) => ({ tag, label: TAG_LABELS[tag]?.label ?? tag, pct }))
  ...
  return (
    <div className="px-4 py-4 space-y-4">
      ...
      <div className="bg-[#0d0d18] border border-white/8 rounded-xl p-4">
        <p className="text-xs font-bold text-white/40 uppercase tracking-wider mb-3">Win vs Loss Comparison</p>
      ...
      {winTagRows.length > 0 && (
        <div className="bg-[#0d0d18] border border-emerald-500/10 rounded-xl p-4">
          <p className="text-xs font-bold text-emerald-400/60 uppercase tracking-wider mb-3">
            ...
            Win Patterns <span className="font-normal text-white/25">% of wins</span>
          </p>
      ...
      {lossTagRows.length > 0 && (
        <div className="bg-[#0d0d18] border border-white/8 rounded-xl p-4">
          <p className="text-xs font-bold text-white/40 uppercase tracking-wider mb-3">
            ...
            Loss Patterns <span className="font-normal text-white/25">% of losses</span>
          </p>
          <div className="space-y-2">
          ...
          {confirmClear ? (
            <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-3 flex items-center justify-between gap-3">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setConfirmClear(false)} className="text-[11px] font-bold px-3 py-1.5 rounded-lg bg-white/8 text-white/50 active:scale-95">
                  Cancel
                </button>
              </div>
            </div>
          ) : (
            <button onClick={() => setConfirmClear(true)} className="w-full text-[11px] text-white/20 hover:text-red-400 transition-colors py-2 flex items-center justify-center gap-1.5">
      ...
      {/* Note modal for journal entries */}
      {editEntry && (
        <NoteModal
          symbol={editEntry.symbol}
          closedAt={editEntry.closedAt}
          note={editEntry.note ?? ""}
          positionId={editEntry.positionId}
          onSave={(id, note) => editClosedTrade.mutate({ positionId: id, note }, { onSuccess: () => setEditEntry(null) })}
...
export default function Analytics() {
  ...
  const { data: traderStatus }    = useAutoTraderStatus();
  const deleteClosedTrade  = useDeleteClosedTrade();
  const editClosedTrade    = useEditClosedTrade();

  const [confirmDeleteId, setConfirmDeleteId] = useState<string | null>(null);
  const [editingTrade, setEditingTrade] = useState<{ positionId: string; symbol: string; closedAt: string; note: string } | null>(null);
  ...
  const streakColor = analytics.currentStreakType === "win" ? "text-emerald-400" : analytics.currentStreakType === "loss" ? "text-red-400" : "text-white/40";
  const streakIcon = analytics.currentStreakType === "win" ? "🔥" : analytics.currentStreakType === "loss" ? "❄️" : "";
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto">
        {tab === "overview" && (
          <div className="px-4 py-4 space-y-4">
              ...
              <div className="flex items-center gap-5">
                <div className="relative flex-shrink-0">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="text-xl font-black text-white">{winRate.toFixed(0)}<span className="text-sm text-violet-400">%</span></span>
                  </div>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-white/40 text-xs uppercase tracking-widest font-semibold mb-1">Win Rate</p>
                  <p className="text-white/60 text-xs">{analytics.winCount}W / {analytics.lossCount}L — {analytics.totalTrades} trades</p>

                  {/* Streak + Last 10 */}
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    <div className="bg-white/5 rounded-lg px-2.5 py-2">
                      <p className="text-white/30 text-[9px] uppercase tracking-wide mb-0.5">Streak</p>
                      <p className={`font-black text-sm ${streakColor}`}>
                        {analytics.currentStreak > 0 ? `${streakIcon} ${analytics.currentStreak}` : "—"}
                        {analytics.currentStreak > 0 && <span className="text-[10px] ml-1 opacity-60">{analytics.currentStreakType}</span>}
                      </p>
                    </div>
                    <div className="bg-white/5 rounded-lg px-2.5 py-2">
                      <p className="text-white/30 text-[9px] uppercase tracking-wide mb-0.5">Last 10</p>
                      <p className={`font-black text-sm ${analytics.winRateLast10 >= 50 ? "text-emerald-400" : "text-red-400"}`}>
                        {analytics.winRateLast10.toFixed(0)}%
                      </p>
                    </div>
                  </div>
                </div>
            ...
            {/* Market Health */}
            {traderStatus?.marketHealth && (
              <MarketHealthCard health={traderStatus.marketHealth} />
            )}

            {/* Stats grid */}
            <div className="grid grid-cols-2 gap-3">
              <StatCard icon={<TrendingUp className="w-4 h-4 text-emerald-400" />} bg="bg-emerald-500/10"
                label="Total P&L" valueClass={analytics.totalPnlSol >= 0 ? "text-emerald-400" : "text-red-400"}
                value={`${fmtSigned(analytics.totalPnlSol)} SOL`}
                sub={`${fmtSigned(analytics.totalPnlPercent, 2)}% of balance`}
              />
              <StatCard icon={<BarChart2 className="w-4 h-4 text-blue-400" />} bg="bg-blue-500/10"
                label="Today's P&L" valueClass={analytics.dailyPnl >= 0 ? "text-emerald-400" : "text-red-400"}
                value={`${fmtSigned(analytics.dailyPnl)} SOL`}
              />
              <StatCard icon={<Activity className="w-4 h-4 text-violet-400" />} bg="bg-violet-500/10"
                label="Risk/Reward" valueClass="text-violet-400"
                value={analytics.avgRR > 0 ? `${analytics.avgRR.toFixed(2)}x` : "—"}
                sub="avg win / avg loss"
              />
              <StatCard icon={<Zap className="w-4 h-4 text-amber-400" />} bg="bg-amber-500/10"
                label="Profit Factor" valueClass={analytics.profitFactor >= 1 ? "text-emerald-400" : "text-red-400"}
                value={analytics.profitFactor >= 99 ? "∞" : analytics.profitFactor > 0 ? `${analytics.profitFactor.toFixed(2)}x` : "—"}
                sub="gross wins / gross losses"
              />
              <StatCard icon={<Award className="w-4 h-4 text-emerald-400" />} bg="bg-emerald-500/10"
                label="Best Trade" valueClass="text-emerald-400"
                value={analytics.bestTradePnl > 0 ? `+${analytics.bestTradePnl.toFixed(4)} SOL` : analytics.bestTradePnl < 0 ? `${analytics.bestTradePnl.toFixed(4)} SOL` : "—"}
              />
              <StatCard icon={<TrendingDown className="w-4 h-4 text-red-400" />} bg="bg-red-500/10"
                label="Worst Trade" valueClass="text-red-400"
                value={analytics.worstTradePnl !== 0 ? `${analytics.worstTradePnl.toFixed(4)} SOL` : "—"}
              />
              <StatCard icon={<Target className="w-4 h-4 text-teal-400" />} bg="bg-teal-500/10"
                label="Avg Win" valueClass="text-emerald-400"
                value={analytics.avgWinSol > 0 ? `+${analytics.avgWinSol.toFixed(4)} SOL` : "—"}
              />
              <StatCard icon={<Clock className="w-4 h-4 text-white/40" />} bg="bg-white/5"
                label="Avg Hold" valueClass="text-white"
                value={analytics.avgHoldTimeMinutes > 0 ? `${analytics.avgHoldTimeMinutes.toFixed(0)}m` : "—"}
              />
            </div>
            ...
            <div>
              ...
              {closedTrades.length === 0 ? (
                <div className="bg-[#0d0d18] border border-white/8 rounded-xl p-8 text-center">
                  ...
                  <p className="text-white/20 text-xs mt-1">Trades appear here once the bot closes positions</p>
                </div>
              ) : (
                <div className="space-y-2">
                      ...
                      <div key={trade.positionId} className={`bg-[#0d0d18] border rounded-xl p-3.5 ${isWin ? "border-emerald-500/15" : "border-red-500/15"}`}>
                        ...
                        <div className="grid grid-cols-4 gap-2 text-[10px] mb-2">
                          ...
                          <div>
                            ...
                            <p className="text-violet-400 font-bold">{trade.aiScore}</p>
                          </div>
                        </div>

                        {trade.note && (
                          <p className="text-[10px] text-violet-400/70 mb-2 flex items-center gap-1">
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            {isConfirming ? (
                              <div className="flex items-center gap-1.5">
                                ...
                                <button onClick={() => setConfirmDeleteId(null)} className="text-[10px] font-bold px-2 py-1 rounded-md bg-white/8 text-white/50 active:scale-95">
                                  No
                                </button>
                              </div>
                            ) : (
      ...
      {/* Note modal for trade history (overview) */}
      {editingTrade && (
        <NoteModal
          symbol={editingTrade.symbol}
          closedAt={editingTrade.closedAt}
          note={editingTrade.note}
          positionId={editingTrade.positionId}
          onSave={(id, note) => editClosedTrade.mutate({ positionId: id, note }, { onSuccess: () => setEditingTrade(null) })}
